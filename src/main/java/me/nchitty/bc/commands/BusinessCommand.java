...
import me.nchitty.bc.managers.BusinessManager;
...
public class BusinessCommand extends BaseCommand {
  ...
  public void start(Player sender, String businessName) {
    Business b = businessManager.createBusiness(sender, businessName);
  }

  @Subcommand("close|c|delete")
  @Conditions("is-owner")
  public void closePlayer(Player sender) {
    BusinessClosedEvent event
        = new BusinessClosedEvent(Business.BusinessManager.getBusiness(sender));
  ...
  { // TODO create context to get business from input
  ...
  public void balance(CommandSender sender, String[] args) {
    if (sender instanceof Player) {
      ...
      if (Business.BusinessManager.isOwner(playerSender)) {
        Business b = Business.BusinessManager.getBusiness(( playerSender ));
        Message message = new Message("business.balance").setRecipient(playerSender)
            .setBusiness(b);
        message.sendMessage();
      } else {
        Message message = new Message("errors.not_an_owner").setRecipient(playerSender)
  ...
  public void list(CommandSender sender, String[] args) {
    ...
    switch (args[ 0 ].toLowerCase()) {
      ...
      case "balance":
        ...
        new Paginator<Business>(
            "business.list.by_balance",
            Business.BusinessManager.sortByBalance(),
            sender
        )
            .page(page)
            .forEach(Message::sendMessage);
      ...
      case "name":
        ...
        new Paginator<Business>(
            "business.list.by_name",
            Business.BusinessManager.sortByName(),
            sender
        )
            .page(page)
            .forEach(Message::sendMessage);
        ...
        break;
      default:
        new Message("errors.usage", sender).setOther(
            "/business list [<\"id\"|\"balance\"|\"name\">] [<page>]").sendMessage();
        break;
    }
  }
    ...
    if (sender instanceof Player) {
      helpPage = new String[] {
          ...
          ChatColor.BLUE
              + "/business balance"
              + ChatColor.AQUA
              + "Get the balance of your business.",
          ...
          ChatColor.BLUE
              + "/business list [\"balance\"|\"id\"|\"name\"] [page #]: "
              + ChatColor.AQUA
