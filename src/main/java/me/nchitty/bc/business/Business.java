...
public class Business {
  ...
  public static class BusinessManager {
    ...
    public static Business getBusiness(String name) {
      for (Business business : businessList) {
        if (business.getName().equalsIgnoreCase(name)) return business;
      }
      return null;
    }
    /**
    ...
    public static boolean isOwner(Player playerSender) {
      return isOwner(playerSender.getUniqueId());
    }
    ...
    public static IssuerAwareContextResolver<Business, BukkitCommandExecutionContext> getContextResolver() {
      return (executionContext) -> {
        if (executionContext.getSender() instanceof Player) return getBusiness(executionContext.getPlayer());
        else {
          // ID
          if(executionContext.getNumParams() == 1) {
            try {
              int id = Integer.parseInt(executionContext.getFirstArg());
              Business b = getBusiness(id);
              if(b == null) throw new InvalidCommandArgument("There is no business with that ID");
              return b;
            } catch (NumberFormatException nfe) {
              Business b = getBusiness(executionContext.getFirstArg());
              if(b == null) throw new InvalidCommandArgument("There is no business with that name");
              return b;
            }
          }
          Business b = getBusiness(executionContext.joinArgs());
          if(b == null) throw new InvalidCommandArgument("There is no business with that name");
          return b;
        }
      };
