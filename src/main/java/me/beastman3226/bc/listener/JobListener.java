...
public class JobListener implements Listener {
    ...
    public void onCreate(JobCreatedEvent e) {
        if(!e.isCancelled()) {
            try {
                BusinessCore.getInstance().getJobFileManager().editConfig(new FileData().add(e.getID() + ".player", e.getUUID().toString())
        ...
        BusinessCore.getInstance().getLogger().log(Level.INFO, "Job #" + e.getID() + " has been created.");
    }

    @EventHandler(priority = EventPriority.MONITOR)
    public void onClaim(JobClaimedEvent e) {
        if(!e.isCancelled()) {
            BusinessCore.getInstance().getJobFileManager().editConfig(new FileData().add(e.getID() + ".employee", e.getEmployee().getID()));
            BusinessCore.getInstance().getLogger().log(Level.INFO, e.getEmployee().getCurrentJob() + " has been claimed by " + e.getEmployee().getName());
        }
    }
